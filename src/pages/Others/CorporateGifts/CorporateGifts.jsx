...
import corporateGiftsData from "./CorporateData";
...
const CorporateGifts = () => {
  ...
  const currentData = corporateGiftsData.slice(firstIndex, lastIndex);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-4">Corporate Gifts</h1>
      ...
      <div className="flex justify-center mt-8">
        <nav className="flex items-center">
          ...
          {Array.from({
            length: Math.ceil(corporateGiftsData.length / itemsPerPage),
          }).map((item, index) => (
            <button
              key={index}
              className={`px-3 py-2 ${
          ...
          <button
            className={`px-3 py-2 rounded-r ${
              currentPage ===
              Math.ceil(corporateGiftsData.length / itemsPerPage)
                ? "bg-gray-300"
                : "bg-gray-500 text-white hover:bg-gray-600"
            }`}
            onClick={() => handlePageChange(currentPage + 1)}
